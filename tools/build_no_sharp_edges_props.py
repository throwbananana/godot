...
def render_and_clean(objects, out_path):
    ...
    print(f"Rendered No-Sharp-Edges: {os.path.basename(out_path)}")

# ==================== BUILDINGS ====================

def build_no_sharp_edges_turret_base():
    objs = []
    ...
    add_smooth_bevel(b, width=0.08, segments=3)
    objs.append(b)
...
def build_no_sharp_edges_turret_gun():
    objs = []
    ...
    for bx in [-0.22, 0.22]:
        ...
        add_smooth_bevel(barrel, width=0.04, segments=2)
        objs.append(barrel)
...
def build_no_sharp_edges_fortified_wall():
    objs = []
    ...
    add_smooth_bevel(w, width=0.22, segments=4)
    objs.append(w)
    ...
    add_smooth_bevel(c, width=0.06, segments=2)
    objs.append(c)
    return objs

def build_no_sharp_edges_landmine():
    objs = []
    ...
    add_smooth_bevel(m, width=0.08, segments=3)
    objs.append(m)
...
def build_no_sharp_edges_repair_station():
    objs = []
    ...
    add_smooth_bevel(p, width=0.08, segments=3)
    objs.append(p)
...
def main():
    ...
    render_and_clean(build_no_sharp_edges_turret_base(), os.path.join(SPRITES_BUILDINGS, "turret_base.png"))
    render_and_clean(build_no_sharp_edges_turret_gun(), os.path.join(SPRITES_BUILDINGS, "turret_gun.png"))
    render_and_clean(build_no_sharp_edges_fortified_wall(), os.path.join(SPRITES_BUILDINGS, "fortified_wall.png"))
    render_and_clean(build_no_sharp_edges_landmine(), os.path.join(SPRITES_BUILDINGS, "landmine.png"))
    render_and_clean(build_no_sharp_edges_repair_station(), os.path.join(SPRITES_BUILDINGS, "repair_station.png"))
