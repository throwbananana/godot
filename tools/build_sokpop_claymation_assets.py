...
def create_sokpop_warm_tabletop_lighting(ortho_scale=3.3):
    ...
    # 1. Warm Cozy Tabletop Sunlight (Honey Amber)
    ...
    # 2. Soft Ambient Clay Fill (Cream / Peach Ambient)
...
def create_sokpop_clay_mat(name, col, roughness=0.76, emission=None, emission_str=0.0):
    ...
    bsdf.inputs['Metallic'].default_value = 0.0 # 纯正无反光黏土

    # 模拟黏土次表面散射（Subsurface Scattering）柔和透光
...
# ==================== SOKPOP CLAYMATION TANK BUILDER ====================
...
def build_sokpop_tank(name_prefix, col_body, col_turret, col_trim,
    ...
    objs = []
    if is_bonus:
        col_body = (0.95, 0.38, 0.45, 1.0) # Strawberry Clay Pink
        col_turret = (0.98, 0.52, 0.58, 1.0)
        col_trim = (1.0, 0.85, 0.28, 1.0)

    mat_body = create_sokpop_clay_mat(f"{name_prefix}_b", col_body)
    mat_turret = create_sokpop_clay_mat(f"{name_prefix}_t", col_turret)
    mat_track = create_sokpop_clay_mat(f"{name_prefix}_tr", (0.32, 0.30, 0.36, 1.0), roughness=0.85) # Slate Clay
    mat_trim = create_sokpop_clay_mat(f"{name_prefix}_tm", col_trim)
    mat_eyes = create_sokpop_clay_mat(f"{name_prefix}_ey", (0.15, 0.15, 0.2, 1.0), roughness=0.3)
    mat_glow = create_sokpop_clay_mat(f"{name_prefix}_gl", (0.35, 0.9, 1.0, 1.0), emission=(0.35, 0.9, 1.0, 1.0), emission_str=3.5)
    ...
    # 1. Soft Chunky Clay Hull (Squishy Squircle)
    bpy.ops.mesh.primitive_cube_add(size=1.0, location=(0, -0.05, 0))
    hull = bpy.context.active_object
    hull.scale = (w, l, 0.56)
    hull.data.materials.append(mat_body)
    add_smooth_clay_bevel(hull, width=0.18, segments=4)
    objs.append(hull)

    # Cute Little Clay Nose Bumper
    ...
    add_smooth_clay_bevel(nose, width=0.1, segments=3)
    objs.append(nose)

    # Cute Blushing Clay Headlights (Sokpop Toy Eye Touch!)
    ...
    # 2. Pill Tracks & Chunky Dough Wheels
    for side, x_pos in [('L', -tx), ('R', tx)]:
        ...
        add_smooth_clay_bevel(tr, width=0.16, segments=4)
        objs.append(tr)
        ...
        for wy in [-0.45, 0.0, 0.45]:
            ...
            add_smooth_clay_bevel(wh, width=0.08, segments=3)
            objs.append(wh)
        ...
        for i in range(num_treads):
            ...
            add_smooth_clay_bevel(tread, width=0.04, segments=2)
            objs.append(tread)

    # 3. Chubby Sphere Turret & Cute Toy Hatch Knob
    ...
    add_smooth_clay_bevel(hatch, width=0.08, segments=3)
    objs.append(hatch)

    bpy.ops.mesh.primitive_uv_sphere_add(radius=0.08, location=(0, -0.16, 0.92))
    knob = bpy.context.active_object
    knob.data.materials.append(mat_trim)
    bpy.ops.object.shade_smooth()
    objs.append(knob)

    # 4. Cute Chunky Clay Cannons
    if barrel_count == 1:
        ...
        add_smooth_clay_bevel(barrel, width=0.06, segments=3)
        objs.append(barrel)
        ...
        for bx in [-0.24, 0.24]:
            ...
            barrel.rotation_euler = (math.radians(90), 0, 0)
            barrel.data.materials.append(mat_turret)
            add_smooth_clay_bevel(barrel, width=0.05, segments=3)
            objs.append(barrel)
...
def build_sokpop_brick():
    objs = []
    mat_clay = create_sokpop_clay_mat("m_sc_c", (0.92, 0.48, 0.28, 1.0)) # Warm Biscuit Clay
    mat_cream = create_sokpop_clay_mat("m_sc_m", (0.96, 0.92, 0.85, 1.0)) # Cream Mortar
    ...
    add_smooth_clay_bevel(base, width=0.1, segments=3)
    objs.append(base)
    ...
    for r in range(3):
        for c in range(3):
            ...
            add_smooth_clay_bevel(b, width=0.18, segments=4)
            objs.append(b)
    return objs

def build_sokpop_steel():
    objs = []
    mat_plate = create_sokpop_clay_mat("m_ss_p", (0.80, 0.85, 0.92, 1.0)) # Soft Slate Clay
    mat_gold = create_sokpop_clay_mat("m_ss_g", (0.98, 0.82, 0.22, 1.0))
    mat_stripe = create_sokpop_clay_mat("m_ss_s", (1.0, 0.75, 0.15, 1.0))

    bpy.ops.mesh.primitive_cube_add(size=1.0, location=(0, 0, 0))
    plate = bpy.context.active_object
    plate.scale = (2.85, 2.85, 0.35)
    plate.data.materials.append(mat_plate)
    add_smooth_clay_bevel(plate, width=0.2, segments=4)
    objs.append(plate)
    ...
    add_smooth_clay_bevel(inner, width=0.08, segments=3)
    objs.append(inner)
...
def build_sokpop_water(frame=0):
    objs = []
    mat_water = create_sokpop_clay_mat("m_sw_w", (0.25, 0.72, 0.92, 1.0), roughness=0.35) # Soft Blue Water
    mat_foam = create_sokpop_clay_mat("m_sw_f", (0.98, 0.98, 1.0, 1.0))

    bpy.ops.mesh.primitive_cube_add(size=1.0, location=(0, 0, 0))
    water = bpy.context.active_object
    water.scale = (2.85, 2.85, 0.3)
    water.data.materials.append(mat_water)
    add_smooth_clay_bevel(water, width=0.15, segments=3)
    objs.append(water)
    ...
    for fx, fy in [(-0.8, -0.6 + offset), (0.6, 0.4 + offset), (-0.2, 0.7 - offset), (0.3, -0.7 + offset)]:
        ...
        add_smooth_clay_bevel(f, width=0.04, segments=2)
        objs.append(f)
    return objs

def build_sokpop_trees():
    objs = []
    mat_matcha = create_sokpop_clay_mat("m_st_m", (0.36, 0.76, 0.32, 1.0))
    mat_lime = create_sokpop_clay_mat("m_st_l", (0.55, 0.85, 0.35, 1.0))
    mat_berry = create_sokpop_clay_mat("m_st_b", (0.95, 0.35, 0.45, 1.0))
...
def build_sokpop_ice():
    objs = []
    mat_sugar = create_sokpop_clay_mat("m_si_s", (0.85, 0.95, 0.98, 1.0), roughness=0.25)
    mat_sparkle = create_sokpop_clay_mat("m_si_sp", (1.0, 1.0, 1.0, 1.0), emission=(1.0, 1.0, 1.0, 1.0), emission_str=1.8)

    bpy.ops.mesh.primitive_cube_add(size=1.0, location=(0, 0, 0))
    ice = bpy.context.active_object
    ice.scale = (2.85, 2.85, 0.3)
    ice.data.materials.append(mat_sugar)
    add_smooth_clay_bevel(ice, width=0.22, segments=4)
    objs.append(ice)
    ...
    for (px, py, l, angle) in [(-0.4, -0.3, 0.9, 30), (0.4, 0.4, 0.7, -40)]:
        ...
        add_smooth_clay_bevel(s, width=0.04, segments=2)
        objs.append(s)
    return objs

def build_sokpop_eagle(destroyed=False):
    objs = []
    mat_ped = create_sokpop_clay_mat("m_se_p", (0.88, 0.82, 0.75, 1.0) if not destroyed else (0.42, 0.40, 0.45, 1.0))
    mat_chick = create_sokpop_clay_mat("m_se_c", (1.0, 0.85, 0.22, 1.0)) # Warm Chick Yellow
    mat_beak = create_sokpop_clay_mat("m_se_b", (1.0, 0.55, 0.15, 1.0))
    mat_blush = create_sokpop_clay_mat("m_se_bl", (1.0, 0.45, 0.55, 1.0)) # Soft Pink Blush!
    mat_eyes = create_sokpop_clay_mat("m_se_e", (0.15, 0.15, 0.2, 1.0), roughness=0.2)
    mat_sparkle = create_sokpop_clay_mat("m_se_sp", (1.0, 1.0, 1.0, 1.0), emission=(1.0, 1.0, 1.0, 1.0), emission_str=2.5)

    # Soft Pill Base Altar
    ...
    add_smooth_clay_bevel(ped, width=0.12, segments=3)
    objs.append(ped)
    ...
    if not destroyed:
        ...
        add_smooth_clay_bevel(beak, width=0.06, segments=2)
        objs.append(beak)
        ...
        for sx in [-0.28, 0.28]:
            ...
            # Sokpop Signature Pink Cheeks (Blushing Dots!)
...
def main():
    ...
    create_sokpop_warm_tabletop_lighting(ortho_scale=3.3)

    print(">>> 1. Sokpop Clay: Rendering Player Tanks...")
    player_palettes = {
        # Tier 0: Mustard Honey Yellow + Grass Green Trim
        ...
        # Tier 1: Apricot Peach Warm Orange
        ...
        # Tier 3: Periwinkle Sky Cerulean & Gold
    ...
    print(">>> 2. Sokpop Clay: Rendering Enemy Tanks...")
    enemies = {
    ...
    print(">>> 3. Sokpop Clay: Rendering Map Tiles...")
    tiles = {
        ...
        "tile_water.png": lambda: build_sokpop_water(0)
    }
    ...
    print(f"Master Sokpop Clay .blend Saved: {BLENDER_SAVE}")
