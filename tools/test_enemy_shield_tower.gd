...
func _test_tower_assets_and_initialization() -> void:
	...
	for f in range(4):
		...
		check(tex != null, "Tower frame %d must exist" % f)

	var primary = TextureHelper.get_tex("res://assets/sprites/buildings/enemy_shield_tower.png")
	check(primary != null, "Tower primary texture must exist")

	var tower_scene = load("res://scenes/buildings/enemy_shield_tower.tscn")
	if not check(tower_scene != null, "EnemyShieldTower scene must exist"):
		return

	var tower = tower_scene.instantiate()
	root.add_child(tower)

	check(tower.is_in_group("enemy_building"), "Tower must be in enemy_building group")
	check(tower.max_hp >= 8, "Tower max HP should be >= 8, got %d" % tower.max_hp)
	check(tower.shield_radius >= 150.0, "Tower shield radius should be >= 150.0")
...
func _test_shield_protection_and_dispell_mechanic() -> void:
	...
	check(enemy.is_shielded() == true, "Enemy inside tower radius MUST be shielded!")
	check(enemy.shield_sources.has(tower), "Enemy must have tower in shield_sources")
	...
	enemy.take_damage(2)
	check(enemy.health == initial_hp, "Shielded enemy MUST NOT take damage! HP was %d, expected %d" % [enemy.health, initial_hp])
	...
	check(enemy.is_shielded() == false, "Enemy outside radius MUST NOT be shielded!")
	print("  [PASS] Enemy exited shield radius (is_shielded=false).")

	# Enemy now takes damage normally
	enemy.take_damage(1)
	check(enemy.health == initial_hp - 1, "Unshielded enemy MUST take damage normally! HP was %d" % enemy.health)
	...
	tower._on_shield_area_entered(enemy)
	check(enemy.is_shielded() == true, "Enemy back in range MUST regain shield!")

	# 6. Destroy the Enemy Shield Tower -> buff MUST be cancelled immediately!
	# GDScript lambdas capture enclosing local variables BY VALUE, not by reference:
	# `var destroyed_signal_fired = false; ...connect(func(): destroyed_signal_fired = true)`
	# mutates a copy inside the closure and never writes back to the outer variable,
	# so the assertion below would always see `false` even though the signal fires
	# correctly. Wrapping the flag in an Array (a reference type) lets the closure
	# mutate shared state that the outer scope can actually observe.
	var destroyed_signal_fired = [false]
	tower.tower_destroyed.connect(func(): destroyed_signal_fired[0] = true)
	tower.take_damage(999)

	check(destroyed_signal_fired[0] == true, "Tower must emit tower_destroyed signal")
	check(enemy.is_shielded() == false, "Enemy shield MUST be cancelled when tower is destroyed!")
	check(enemy.shield_sources.is_empty(), "Enemy shield_sources MUST be empty after tower destruction")
...
func _test_encyclopedia_integration() -> void:
	...
	for b in blds:
		if b.get("id") == "bld_enemy_shield_tower":
			found_tower = true
			check(TextureHelper.get_tex(b.get("icon")) != null, "Tower icon must be loadable")
			check(not b.get("desc", "").is_empty(), "Tower desc must exist")
			check(not b.get("tactics", "").is_empty(), "Tower tactics must exist")

	check(found_tower, "Compendium MUST contain bld_enemy_shield_tower entry")
