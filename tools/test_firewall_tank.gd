...
func _test_firewall_tank_assets_and_instantiation() -> void:
	...
	for path in required_frames:
		var tex = load(path)
		check(tex != null, "火墙坦克/火焰贴图必须存在: %s" % path)
...
func _test_firewall_tank_stats() -> void:
	...
	check(enemy.enemy_type == EnemyTank.EnemyType.FIREWALL, "敌人类型必须为 FIREWALL")
	check(enemy.max_health >= 4, "火墙坦克基础生命值必须 >= 4")
	check(enemy.MAX_FIREWALL_TRAIL_LENGTH == 10, "火墙轨迹队列上限必须严格为 10 格")
	check(enemy.FIREWALL_DROP_DISTANCE > 0, "火墙铺设距离间隔必须大于 0")
...
func _test_firewall_trail_10_cap_queue() -> void:
	...
	# 验证火墙队列数量被严格限制在 10 格
	check(enemy.firewall_trail_queue.size() == 10, "火墙队列数量必须被严格限制在 10 格, 实际为 %d" % enemy.firewall_trail_queue.size())

	# 验证所有火墙均有效存在
	for fire in enemy.firewall_trail_queue:
		if check(is_instance_valid(fire), "队列中的火墙必须为有效实例"):
			check(fire.is_in_group("firewall"), "火墙必须属于 firewall 组")
...
func _test_firewall_touch_damage() -> void:
	...
	if not check(hazard_scene != null, "firewall_hazard.tscn 场景必须存在"):
		return
	var fire = hazard_scene.instantiate()
	root.add_child(fire)

	# 用真实的 player.tscn 实例，而不是 Node2D.new() + set_script(player.gd)。
	# player.gd 是 `extends CharacterBody2D`，用错底层节点类型会导致 set_script()
	# 被 Godot 静默拒绝，后续 .set("health", ...) 落空——而且 player.gd 上根本没有
	# "health" 字段，真正的属性叫 current_health。另外 player.gd::_ready() 无条件
	# 调用 set_invulnerable(3.5)（CLAUDE.md 里点名过的坑），刚生成的玩家会直接
	# 无视 take_damage()，测试前必须手动关掉无敌状态。
	var mock_player = PlayerScene.instantiate()
	mock_player.player_id = 1
	root.add_child(mock_player)
	mock_player.is_invulnerable = false
	var hp = 3
	mock_player.current_health = hp

	# 触发触碰伤害
	fire._apply_burn_damage(mock_player)
	check(mock_player.current_health == hp - 1, "触碰火墙必须扣除 1 点生命值, 实际为 %d" % mock_player.current_health)
...
func _test_firewall_cleanup_on_death() -> void:
	...
	check(enemy.firewall_trail_queue.size() == 5, "生成了 5 个火墙")

	# 触发死亡
	enemy._die()

	check(enemy.firewall_trail_queue.is_empty(), "死亡后火墙队列必须被清空")
...
func _test_encyclopedia_and_metadata() -> void:
	...
	for entry in EncyclopediaDataScript.ENTRIES:
		if entry.get("id") == "enemy_firewall":
			...
			check("10" in entry.get("stats", {}).get("特殊能力", ""), "图鉴属性必须标明 10 格火墙轨迹")
			break

	check(found, "图鉴中必须包含 enemy_firewall 条目")
