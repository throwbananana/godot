...
## 56-69 是同一批地块的绿色变体/双色 AND 变体/保持型压力板系统, 分类规则
## 完全照搬——57(炸开关绿)/58(受控电墙绿)/60(能量墙绿)/61/63(AND 电墙/能量墙)
## /67-69(闸门, 出生即关闭挡人) 都是 StaticBody2D 挡坦克; 56(压力板绿)/
## 59(受控充能站绿)/62(AND 充能站)/64-66(保持型压力板) 都是 Area2D 纯触发,
## 刻意不列进来。
const HARD_BLOCK := [2, 3, 25, 48, 49, 52, 53, 54, 55, 57, 58, 60, 61, 63, 67, 68, 69]
...
static func validate(grid: Array, tier: int = -1) -> Array:
	...
	for r in range(13):
		...
		for c in range(13):
			...
			# validate() 也会把整张图当"地形号越界"打回去重摇。56-69 (绿色
			# 电路变体/双色 AND 受控物/保持型压力板系统) 上限跟着挪到 69,
			# 免得同一个坑再踩一次。
			if v < 0 or v > 69:
