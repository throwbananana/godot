...
const FAMILY_TILES := {
	...
	# 地块 27-44 (信号干扰塔起) 曾经完全不在这张表里 —— 不是刻意排除, 是这批
	# 结构陆续接进手写模板和 main.gd 的分派梯之后, 没人回头把它们也补进程序
	# 生成的族表。后果比表面看起来大: MapDirector.validate() 曾经把地形号上限
	# 硬编码在 29 (见下方 v > 44 那处改动), 而 FAMILY_TILES 又是 Daily
	# Challenge 和三分之一常规战楼层 (floor_idx % 3 == 2) 生成地图时**唯一**的
	# 机制地块来源 —— 两条一起，这 12 种地块此前在程序生成里一格都不会出现，
	# 哪怕它们在手写模板和百科全书里都是正经内容。
	"jammer":      [27],
	"factory":     [28],
	"drift":       [29],
	"shield_tower": [30],
